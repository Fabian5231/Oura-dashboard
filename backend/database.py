...
_VALID_TABLES = frozenset({
    ...
    "daily_resilience", "sleep", "heartrate",
})
...
def get_cardiovascular_age(start: str | None = None, end: str | None = None) -> list[dict]:
    with _db() as conn:
        if start and end:
            rows = _rows_to_dicts(conn.execute(
                "SELECT day, vascular_age FROM daily_cardiovascular_age WHERE day BETWEEN ? AND ? ORDER BY day",
                (start, end)).fetchall())
        else:
            rows = _rows_to_dicts(conn.execute(
...
def get_overview() -> dict:
    ...
    with _db() as conn:
        # Table names are hardcoded — no user input, safe for f-string
        tables = {
